...
function checkPrereqs() {
  ...
  chmod 777 /home/rohit/workspace/delivery-network/network/bin/configtxlator
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of delivery-network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of delivery-network."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q "$UNSUPPORTED_VERSION"
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of delivery-network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of delivery-network."
      exit 1
    fi
  done
}
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org.delivery-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/org_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile DeliveryChannel -outputAnchorPeersUpdate ./channel-artifacts/orgMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg orgMSP
  res=0
  set +x
  if [ $res -ne 0 ]; then
